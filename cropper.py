...
class AdvancedCropper:
    ...
    def load_image(self, path):
        ...
        self.original_format = (self.original.format or "PNG").upper()
    ...
    def save_crop(self):
        ...
        # Ако потребителят не е написал разширение – добавяме според избрания филтър
        lower = path.lower()
        if not (lower.endswith(".png") or lower.endswith(".jpg") or lower.endswith(".jpeg") or lower.endswith(".webp")):
            # fallback според оригиналния формат
        ...
        try:
            if path.lower().endswith((".jpg", ".jpeg")):
                ...
                cropped.save(path, format="JPEG", quality=100, subsampling=0, optimize=False)

            elif path.lower().endswith(".webp"):
                # Запазваме с максимално качество (lossless ако има прозрачност)
                if cropped.mode in ("RGBA", "P"):
                    cropped.save(path, format="WEBP", lossless=True)
                else:
                    cropped.save(path, format="WEBP", quality=100)

            else:  # PNG
                cropped.save(path, format="PNG", compress_level=0)
